...
spl=Spline1D(Drange,ff.-maximum(ff),w=ones(length(Drange)),k=3,bc="nearest",s=0.3)
...
function univariateλ(λ)
    ...
    spl=Spline1D(λrange,ff.-maximum(ff),w=ones(length(λrange)),k=3,bc="nearest",s=5.0)
